...
**Package:** `@grant-vine/wunderkind` v0.24.0  
...
- **Prompt optimization in this repo is a local override, not a product default** — the product contract stays supplementary/default-off, but this repository currently keeps a project-local `.wunderkind/wunderkind.config.jsonc` override with `promptOptimizationMode: "active"`, `promptOptimizationReportingMode: "summary"`, `promptOptimizationTokenBudget: 120000`, and `promptOptimizationByteBudget: 1200`. Treat that as local repo state for this explicitly enabled repo context, not a published default.
- **V4 safe user-prompt optimization is tightly bounded** — in enabled contexts only, V4 may optimize the latest user-authored message. Retained history, earlier user messages, SOUL overlays, and transcript-wide compaction content are excluded. Code blocks, URLs, paths, commands, explicit requirements, compliance/legal/security wording, and quoted user text stay byte-exact. Low-confidence or safety-risk cases pass the whole message through unchanged.
- **Runtime optimization reports live outside the audit-only `token-audit` surface** — latest-only runtime artifacts live at `.wunderkind/runtime/prompt-optimization/system-transform.latest.json` and `.wunderkind/runtime/prompt-optimization/session-compacting.latest.json`; `doctor --verbose` reports posture and artifact existence, not proven savings. V4 passthrough reasons are runtime-report-only and must not be documented as summary metadata guidance.
