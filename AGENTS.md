...
**Package:** `@grant-vine/wunderkind` v0.27.6
...
├── skills/                    # Static SKILL.md files: 23 promoted + 4 Wunderkind-specific public routes, 1 deprecated docs-history route, and SKILL-STANDARD.md
...
└── oh-my-openagent.jsonc      # Canonical agent registration config (category, color, mode per agent)
```
...
| Change agent category defaults | `oh-my-openagent.jsonc` |
...
| Audit current skill ownership/bucket | `skills/SKILL-STANDARD.md` |
...
- **Delegation Contract (Maintainers):** When writing task examples in docs, agents, or skills, always include both required fields:
  ...
  Historical `.sisyphus/**` archives are migration history only and are intentionally excluded from active delegation examples.
...
Sub-skills: current repo-head public routes are the 23 promoted retained-specialist skills plus 4 Wunderkind-specific workflow skills from `skills/SKILL-STANDARD.md` (`promoted=23`, `wunderkind-specific=4`, `deprecated=1`, `public/deprecated total=28`): `social-media-maven` + `technical-writer` (marketing-wunderkind) · `visual-artist` (creative-director) · `agile-pm` + `grill-me` + `docs-with-grill` + `setup-wunderkind-workflow` + `ubiquitous-language` + `prd-pipeline` + `release-upgrade` + `triage-issue` + `experimentation-analyst` + `caveman` + `write-a-skill` (product-wunderkind) · `db-architect` + `diagnose` + `code-health` + `platform-compatibility` + `supportability-review` + `vercel-architect` + `supabase-architect` + `improve-codebase-architecture` + `tdd` (fullstack-wunderkind) · `security-analyst` + `pen-tester` + `compliance-officer` (ciso) · `oss-licensing-advisor` (legal-counsel). Deprecated skill route: `design-an-interface` is documentation and detection-only; use `improve-codebase-architecture` for structural interface work, direct `fullstack-wunderkind` judgement for narrow engineering decisions, or product/frontend exploration when workflow or prototype evidence shapes the contract.
...
# Gitignore helper
node bin/wunderkind.js gitignore     # add .wunderkind/, AGENTS.md, .omo/, .opencode/ to .gitignore
```
...
- **Published baseline vs compatibility-wave contract** — the current beta-channel baseline is package `0.27.6` on OpenCode `1.18.31` / OMO `5.0.0-beta.62`. OMO v5 beta is in scope, `supportability-review` is part of the shipped promoted surface, and high-capability routing uses `openai/gpt-6-astra`.
...
- **Prompt optimization in this repo is a local override, not a product default** — the product contract stays supplementary/default-off, but this repository currently keeps a project-local `.wunderkind/wunderkind.config.jsonc` override with `promptOptimizationEnabled: true`, `promptOptimizationMode: "active"`, `promptOptimizationReportingMode: "summary"`, `promptOptimizationTokenBudget: 120000`, and `promptOptimizationByteBudget: 1200`. Because `promptOptimizationLevel` is intentionally omitted, this repo currently resolves through the frozen legacy compatibility profile until an operator explicitly chooses a concrete level. Treat that as local repo state for this explicitly enabled repo context, not a published default.
- **The final optimization engine is one capability-based multi-level system** — supported explicit levels are `latest-user`, `runtime-and-tools`, `contextual`, and `transcript`; they are cumulative, and enabling optimization at all keeps the security-safe baseline on (redacted reporting, preserve/fallback enforcement, and no protected-content persistence drift).
- **The latest-user level stays tightly bounded** — in enabled contexts only, the lowest level may optimize the latest user-authored message. Retained history, earlier user messages, SOUL overlays, and transcript-wide compaction content are excluded there. Code blocks, URLs, paths, commands, explicit requirements, compliance/legal/security wording, and quoted user text stay byte-exact. Low-confidence or safety-risk cases pass the whole message through unchanged.
- **Runtime optimization reports live outside the audit-only `token-audit` surface** — latest-only runtime artifacts live at `.wunderkind/runtime/prompt-optimization/system-transform.latest.json` and `.wunderkind/runtime/prompt-optimization/session-compacting.latest.json`; `doctor --verbose` reports posture and artifact existence, not proven savings. Latest-user passthrough reasons are runtime-report-only and must not be documented as summary metadata guidance.
- **Persistent memory and automatic context injection remain unsupported** — no persistent cross-session memory writes and no automatic context injection belong to the final optimization contract.
...
```bash
bun test tests/unit/                          # full unit suite; rerun for current pass count
...
3. Prefer canonical upstream names in new output. Preserve legacy wording only as detection-only migration history where the hard-cut contract requires it.
