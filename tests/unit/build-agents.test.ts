...
describe("build-agents script", () => {
  ...
  it("keeps high-signal generated prompt surfaces in the expected markdown shape", () => {
    ...
    expect(marketing).toContain("SOUL files are read-only")
    expect(marketing).toContain("Every slash command must support a `--help` form.")
    expect(marketing).toContain("write: deny")
    ...
    expect(ciso).toContain("write: deny")
    expect(ciso).toContain("edit: deny")
    expect(ciso).toContain("apply_patch: deny")
    expect(ciso).toContain("bounded durable-artifact writer")
    expect(ciso).not.toContain("append via shell")
    expect(fullstack).toContain("# Fullstack Wunderkind — Soul")
    expect(fullstack).not.toContain("write: deny")
    expect(fullstack).not.toContain("edit: deny")
    expect(fullstack).not.toContain("apply_patch: deny")
